...
public class Magicienne extends Personnage {
        ...
        for (int i = 0; i < this.getPlateau().getNombreJoueurs(); i++) {
            if (this.getPlateau().getJoueur(i) != null) {
                if (this.getPlateau().getJoueur(i).getPersonnage() != null) {
                    System.out.println(i + 1 + " : Le Personnage " + this.getPlateau().getJoueur(i).getPersonnage() + " a " + this.getPlateau().getJoueur(i).nbQuartiersDansMain() + " Cartes");
                }
    ...
    public void utiliserPouvoir() {
        ...
        if (yesOrNo) {
            ...
            do {
                ...
                int choix = Interaction.lireUnEntier(1, this.getPlateau().getNombreJoueurs(), "Votre choix entre: ");
                //�change les mains des joueurs


                if (this.getPlateau().getJoueur(choix-1).getPersonnage().equals(this)) {
                    System.out.println("Vous ne pouvez pas vous choisir vous m�me. Choisissez un autre joueur.");

                } else {
                        echangeDeListe(this.getPlateau().getJoueur(choix-1).getMain(), this.getJoueur().getMain());
                        continu = false;
                }
            } while (continu);
            ...
            if (this.getJoueur().nbQuartiersDansMain() == 0) {
                ...
                // Si le nombre de cartes � �changer est �gal au nombre de cartes dans la main du joueur, on �change toutes les cartes
                    ...
                    System.out.println("Vous avez echange toutes vos cartes");
    ...
    public void utiliserPouvoirAvatar() {
        ...
        if (yesOrNo) {
            ...
            do {
                int choix = new Random().nextInt(this.getPlateau().getNombreJoueurs());
                System.out.println("Avec qui voulez vous �changer vos cartes ?");
                // r�cup�re le choix de l'utilisateur
                listePersonnagesDisponibles();
                ...
                if (this.getPlateau().getJoueur(choix).getPersonnage().getRang() == 3) {
                    ...
                    if (this.getPlateau().getJoueur(choix).getPersonnage() != null) {
                        echangeDeListe(this.getPlateau().getJoueur(choix).getMain(), this.getJoueur().getMain());
                        continu = false;
                    }
                }
            } while (continu);
